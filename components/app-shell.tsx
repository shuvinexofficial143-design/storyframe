...
import {BookOpen,Boxes,Clapperboard,Film,GalleryHorizontalEnd,Home,Loader2,MapPinned,Settings,Sparkles,Users,WandSparkles} from "lucide-react";
...
const nav=[["/","Continuity Studio",Home],["/story-generator","Story Generator",WandSparkles],["/projects","Legacy Projects",Boxes],["/story","Legacy Story",BookOpen],["/characters","Legacy Characters",Users],["/locations","Legacy Locations",MapPinned],["/scenes","Legacy Scenes",Clapperboard],["/storyboard","Legacy Storyboard",GalleryHorizontalEnd],["/timeline","Legacy Timeline",Film],["/settings","Settings",Settings]] as const;
...
export function AppShell({children}:{children:React.ReactNode}){
  ...
  return <div className="min-h-screen lg:grid lg:grid-cols-[260px_1fr]">
    ...
    <main className="min-w-0"><header className="sticky top-0 z-20 flex min-h-16 items-center justify-between gap-3 border-b border-white/8 bg-white/90 px-4 py-3 text-slate-900 backdrop-blur-xl md:px-7"><div className="min-w-0"><div className="text-sm font-semibold">{path==="/"?"Manga Studio":path==="/story-generator"?"Story Generator":project.storyTitle||project.name}</div><div className="truncate text-xs text-slate-500">{mangaJob.busy?(mangaJob.progress||"Manga job is still running…"):(path==="/"?"All workspace navigation stays available here.":`${project.chapter} · ${project.aspectRatio}`)}</div></div><div className="flex shrink-0 items-center gap-3">{mangaJob.busy&&<Link href="/" className="inline-flex items-center gap-1.5 rounded-lg bg-violet-100 px-2.5 py-1.5 text-[11px] font-semibold text-violet-700"><Loader2 className="animate-spin" size={13}/> Manga running</Link>}{path!=="/"&&<Link href="/" className="text-xs font-semibold text-violet-600">Open Manga Studio</Link>}</div></header><div className="gridbg min-h-[calc(100vh-4rem)] p-4 md:p-7"><div className={path==="/"?"block":"hidden"}><AnalysisModelSelector/><StoryFrameWorkspace/></div><div className={path==="/story-generator"?"block":"hidden"}><StoryGeneratorWorkspace/></div>{path!=="/"&&path!=="/story-generator"&&children}</div></main>
  </div>;
}
