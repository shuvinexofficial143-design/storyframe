  ...
  return <div className="border-b border-slate-200 bg-white px-4 py-3 text-slate-900">
    <div className="mx-auto flex max-w-7xl flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
      <div className="flex items-center gap-3">
        ...
        <div><div className="text-xs font-bold uppercase tracking-[.16em] text-slate-500">AI Story Model</div><div className="text-xs text-slate-600">{selected.description}</div></div>
      </div>
      <select aria-label="AI Story Model" value={model} onChange={(event)=>void changeModel(event.target.value)} className="min-w-[240px] rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm text-slate-900 outline-none focus:border-violet-500">
