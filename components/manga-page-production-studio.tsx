...
export function MangaPageProductionStudio(){
  ...
  const renderPage=async(pageId:string,stronger=false,signal?:AbortSignal)=>{
    ...
    const finalPrompt=pagePromptOverrides[pageId]?.trim()||compiled.prompt;
  ...
  return <main className="min-h-screen bg-[#080a0f] text-zinc-100">
    <div className="mx-auto max-w-7xl space-y-5 px-4 py-6">
      ...
      {tab==="characters"&&<section className="grid gap-4 md:grid-cols-2">{project.characters.length?project.characters.map((character)=><article key={character.id} className="rounded-2xl border border-white/10 bg-[#0d1017] p-4">
        ...
        <div className="mt-4 flex flex-wrap gap-2">{(["primary","full-body","three-quarter","side","sheet"] as ReferenceView[]).map((view)=><button key={view} onClick={()=>void generateReference(character,view)} disabled={!!busy} className="rounded-lg border border-white/10 px-3 py-2 text-xs disabled:opacity-50">{view}</button>)}{busy.startsWith(`ref-${character.id}-`)&&<button onClick={cancelCurrentTask} className="rounded-lg border border-red-500/30 bg-red-500/10 px-3 py-2 text-xs font-bold text-red-200">Cancel</button>}</div>
      ...
      {tab==="pages"&&<section className="space-y-6">
        {production?.pages.length?<>
          ...
          {production.pages.map((page)=><article key={page.id} className="rounded-2xl border border-white/10 bg-[#0d1017] p-4">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><div className="text-lg font-bold">Page {page.pageNumber} <span className="ml-2 text-xs font-normal text-violet-300">{page.panels.length} panels</span></div><div className="mt-1 inline-flex rounded-md border border-violet-500/20 bg-violet-500/10 px-2 py-1 text-[11px] font-semibold text-violet-200">{chapter.title} · Page {page.pageNumber}</div><div className="mt-2 text-xs text-zinc-500">{page.pagePurpose} · {page.panelLayout}</div>{page.renderProvider&&<div className="mt-1 text-[11px] text-zinc-600">{page.renderProvider} · {page.renderModel}</div>}</div><div className="flex flex-wrap gap-2"><button disabled={!!busy} onClick={()=>void generatePage(page.id)} className="rounded-lg bg-violet-500 px-3 py-2 text-xs font-bold disabled:opacity-50">{busy===page.id?<Loader2 className="mr-1 inline animate-spin" size={14}/>:<ImageIcon className="mr-1 inline" size={14}/>} {page.composedImageDataUrl?"Regenerate Page":"Generate Page"}</button>{busy===page.id&&<button onClick={cancelCurrentTask} className="rounded-lg border border-red-500/30 bg-red-500/10 px-3 py-2 text-xs font-bold text-red-200">Cancel</button>}<button disabled={!!busy} onClick={()=>void generatePage(page.id,true)} className="rounded-lg border border-white/10 px-3 py-2 text-xs disabled:opacity-50">Strong Continuity Retry</button><button disabled={!!busy} onClick={()=>{const pageIndex=production.pages.findIndex((item)=>item.id===page.id);const previousPage=pageIndex>0?production.pages[pageIndex-1]:getPreviousRenderedMangaPage(project,chapter.id);const compiled=compileMangaPagePrompt({project,production,page,previousPage,stronger:false});setPagePromptOverrides((value)=>({...value,[page.id]:value[page.id]??compiled.prompt}));setPromptEditorPageId((value)=>value===page.id?null:page.id)}} className="rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs font-semibold text-amber-200 disabled:opacity-50">Edit Image Prompt</button>{page.composedImageDataUrl&&<button disabled={!!busy} onClick={()=>downloadDataUrl(page.composedImageDataUrl!,`${safeFilePart(project.name)}-chapter-${pad(chapterNumber,2)}-page-${pad(page.pageNumber,3)}.jpg`)} className="rounded-lg border border-emerald-500/30 px-3 py-2 text-xs text-emerald-300 disabled:opacity-50"><Download className="mr-1 inline" size={13}/> Download</button>}</div></div>
            {promptEditorPageId===page.id&&<div className="mt-4 rounded-xl border border-amber-500/20 bg-amber-500/5 p-3"><div className="mb-2 flex items-center justify-between gap-2"><div><div className="text-sm font-bold text-amber-200">Final Image Prompt</div><div className="text-xs text-zinc-500">यह वही prompt है जो Generate Page दबाने पर image API को भेजा जाएगा। Sensitive detail हटाकर safe visual wording लिख सकते हो।</div></div><button disabled={!!busy} onClick={()=>setPagePromptOverrides((value)=>{const next={...value};delete next[page.id];return next})} className="rounded-lg border border-white/10 px-2 py-1 text-xs text-zinc-300 disabled:opacity-50">Reset Auto Prompt</button></div><textarea disabled={!!busy} value={pagePromptOverrides[page.id]||""} onChange={(event)=>setPagePromptOverrides((value)=>({...value,[page.id]:event.target.value}))} className="min-h-64 w-full rounded-lg border border-white/10 bg-black/30 p-3 text-xs leading-5 text-zinc-100" spellCheck={false}/><div className="mt-2 text-[11px] text-zinc-500">Manual prompt केवल इस page की अगली generation पर लागू होगा।</div></div>}
