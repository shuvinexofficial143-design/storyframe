...
export type MangaLocationProfile={
  id:string;
  name:string;
  architecture:string;
...
export type MangaTimelineEvent={
  id:string;
  sourceText:string;
  event:string;
  timeOfDay:string;
...
export type MangaContinuityState={
  currentPage:number;
  timeline:string;
  timeOfDay:string;
  currentLocation:string;
  characters:Record<string,MangaCharacterState>;
  activeProps:string[];
  previousPageEndState:string;
};

export type StoryCoverageReport={
  percent:number;
  coveredBeatIds:string[];
  missingBeats:Array<{beatId:string;storyBeat:string;sourceText:string}>;
};

export type MangaChapterProduction={
  schemaVersion:1;
  stylePreset:MangaStylePreset;
  storySummary:string;
  timeline:MangaTimelineEvent[];
  beats:MangaStoryBeat[];
  locationProfiles:MangaLocationProfile[];
  propStates:MangaPropState[];
  initialCharacterStates:Record<string,MangaCharacterState>;
  pages:MangaPage[];
  nextBeatIndex:number;
  continuityState:MangaContinuityState;
...
export type MangaMasterCharacter={
  ...
  face:{shape:string;eyes:string;eyebrows:string;nose:string;mouth:string;specialFeatures:string};
  hair:{color:string;style:string;length:string};
  body:{build:string;height:string;proportions:string};
...
export type MangaMasterAnalysis={
  storySummary:string;
  characters:MangaMasterCharacter[];
  locations:MangaLocationProfile[];
  props:MangaPropState[];
  timeline:MangaTimelineEvent[];
  beats:MangaStoryBeat[];
  initialCharacterStates:Record<string,MangaCharacterState>;
  provider:string;
