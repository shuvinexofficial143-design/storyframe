...
function heuristicAnalyze(story:string,visualStyle:string){
  ...
  for(const sentence of sentences){for(const name of sentence.match(/\b[A-Z][a-z]{2,}(?:\s+[A-Z][a-z]{2,})?\b/g)||[]){if(!stop.has(name))counts.set(name,(counts.get(name)||0)+1)}}
  let names=[...counts].sort((a,b)=>b[1]-a[1]).slice(0,6).map(([name])=>name);if(!names.length)names=["Main Character"];
  ...
  const found=locRules.filter(([,rule])=>rule.test(story)).map(([name])=>name);const locationNames=found.length?found:["Primary Story Location"];
  ...
  const targetSceneCount=Math.min(60,Math.max(4,Math.ceil(story.trim().split(/\s+/).filter(Boolean).length/60)));
  const chunkSize=Math.max(1,Math.ceil(sentences.length/targetSceneCount));const chunks:string[]=[];for(let index=0;index<sentences.length;index+=chunkSize)chunks.push(sentences.slice(index,index+chunkSize).join(" "));
  const scenes=chunks.slice(0,60).map((text,index)=>{const mentionedCharacters=characters.filter((character)=>text.toLowerCase().includes(character.name.toLowerCase()));const refs=mentionedCharacters.length?mentionedCharacters:characters.slice(0,Math.min(2,characters.length));const refIds=refs.map((character)=>character.id);const refNames=refs.map((character)=>character.name).join(", ");const location=locations[Math.min(index,locations.length-1)]||locations[0];const shot=shots[index%shots.length];const angle=angles[index%angles.length];return {id:`scene-${index+1}`,sceneNumber:index+1,sourceText:text,description:text.length>150?`${text.slice(0,147)}...`:text,characterIds:refIds,locationId:location?.id,cameraShot:shot,cameraAngle:angle,duration:Math.max(3,Math.min(6,Math.ceil(text.split(/\s+/).length/10))),imagePrompt:[`${shot}, ${angle}.`,text,`Featured characters: ${refNames||"main character"}.`,`Preserve the established ${location?.name||"location"} layout and important props.`,`Visual style: ${visualStyle}.`,`Cinematic storytelling frame, expressive body language, detailed environment, coherent lighting, strong composition, no text, no captions, no speech bubbles.`].join(" "),negativePrompt:"wrong face, inconsistent outfit, duplicate character, extra limbs, deformed hands, blurry face, text, watermark, logo, inconsistent architecture, cropped head, low quality",continuityNotes:"Carry clothing, held objects, environment layout and emotional state forward from the previous scene.",generationStatus:"idle" as const,generationSeed:hashString(`${text}-${index+1}`)}});
...
export async function POST(req:Request){
  try{
    ...
    if(hasPollinations()){
      try{const analysis=await pollinationsAnalyzeStory({story,visualStyle});const validated=ExternalPayload.parse(analysis.json);if(validated.scenes.length)return NextResponse.json(normalizeExternal(validated,visualStyle,"pollinations"))}catch(error){console.error("Pollinations analysis failed; using local fallback",error)}
    }
    return NextResponse.json(heuristicAnalyze(story,visualStyle));
  }catch(error){console.error("Analyze route failed",error);return NextResponse.json({error:"Failed to analyze story"},{status:500})}
}
