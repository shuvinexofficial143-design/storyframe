...
export async function POST(request:Request){
  try{
    ...
    const analysisModel=liveSelectedModel(request,input.analysisModel);
    ...
    const result=await xkiroVisionJsonCompletion({model:analysisModel,systemPrompt:SYSTEM,userPrompt:prompt,images:[input.image,...input.referenceImages],temperature:0,maxTokens:1800});
    ...
    return NextResponse.json({provider:`xKiro Vision · ${analysisModel}`,...checked.data});
  }catch(error){
