...
export default function Story(){
  ...
  const analyze=async()=>{
    ...
    setError("");
    try{
      const response=await fetch("/api/analyze",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({story:project.story,visualStyle:project.visualStyle,analysisModel:project.analysisModel})});
      ...
      <div className="space-y-5">
        <Card className="p-5">
          ...
          <div className="mt-5 grid gap-3 text-sm">
            ...
            <div className="flex justify-between"><span className="text-zinc-500">Est. scenes</span><span>{Math.max(0,Math.ceil(words/60))}</span></div>
            <div className="flex justify-between gap-3"><span className="text-zinc-500">Analysis provider</span><span className="text-right">{project.analysisProvider||"unknown"}</span></div>
